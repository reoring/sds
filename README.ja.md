...
> **dev-flow** でトラックを選び(軽量が既定、不可逆・顧客可視の作業は
> **prod-flow** へ)、**issue-lane** で issue ごとに lane を1本立てて任せ、
...
| スキル | 何を守らせるか |
|---|---|
| [dev-flow](claude/dev-flow/SKILL.ja.md) | 軽量な既定トラック: 証拠はマージ済み PR + green CI + 台帳一行 — 封緘なし・successor なし・verdict 文書の往復なし。1問の判定(「失敗したら消費財が減るか、顧客に見えるか?」)で重い作業を prod-flow へ振り分ける。live で見つけた問題は再発防止 gate つきでのみ close |
| [prod-flow](claude/prod-flow/SKILL.ja.md) | 不可逆・顧客可視の作業のためのフル規律フロー: 構想 → 事前調査 scout → 設計 → PoC → 確証 → 実装 → 手動での本番反映 → 観察 → 確定。packet 封緘(GO を packet digest に束縛)、工程ごとの receipt、fail-closed な gate、レビュー往復の上限、差し戻し早見表、停止条件つき |
...
2. **計画する — prod-flow 工程 0〜2**(重い経路の場合。日常の作業は dev-flow の
   軽量トラックで進める)。PO が構想メモを書き、読み取り専用の
...
7. **本番に出す — prod-flow 工程 4〜8。** 実行手順の全段を隔離環境で検証しきって
...
- **PO はプロジェクトに1人。** dev-flow / prod-flow で計画し、tracker に issue を起票し、
