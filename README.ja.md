...
| スキル | 強制する規律 |
|---|---|
| [dev-flow](claude/dev-flow/SKILL.ja.md) | 段階フロー: 構想 → read-only scout → 設計 → PoC → 確証 → 実装 → 手動 Live 反映 → 観察 → 確定。工程ごとの receipt、fail-closed な gate、差し戻し表、circuit breaker |
| [issue-lane](claude/issue-lane/SKILL.ja.md) | 1 issue = 1 lane のライフサイクル: tab と pane の両方に issue ID ラベル、issue close で teardown、台帳制のモデルブースト、三点照合の drift 監査(tab × pane × tracker) |
| [herdr-event-watch](claude/herdr-event-watch/SKILL.ja.md) | event-driven な fleet 監督: 耐久 inbox artifact(primary)、lane done/blocked 遷移(backstop)、PR required check の確定 — 固定間隔ポーリングの代替 |
